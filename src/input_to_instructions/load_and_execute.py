...
class Instruction:
    ...
    def __init__(self, instruction):
        ...
        if self.operation_flag in ["q"]:
            ...
            self.semantic: Semantic = Semantic(instruction[4])
    ...
    def __repr__(self) -> str:
        if self.operation_flag == "r":
            ...
            return f"Instruction({[self.operation_flag, self.content, self.save_variable, self.value, self.semantic]})"
...
class InputToInstruction:
    ...
    3. 'Instruction Set': Instruction Set 결과. list 형태로 구성되며, 각 원소는 Instruction과 Semantic으로 구성됨.
    ...
    q tag를 가진 instruction의 경우, instruction[3]에 sql을 위한 세부 문장 외에 다른 문장은 넣지 말것.
...
<좋은예시9>
    ...
    출력: "{
        ...
        "Instruction Set": [
            ["q", "현재 우리반 실내온도 알려줘", "현재 우리반 실내온도", "값", {
                "Temporal": [("현재", "2022-09-30 12:00:00")],
                "Spatial": ["우리반"],
                "Modality": ["실내온도"],
                "Operation": [None],
