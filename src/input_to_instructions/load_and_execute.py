...
따라서 세부 문장은 첫번째, 오늘 우리집의 온도를 알려줘. 두번째, 옆집의 온도를 알려줘. 그리고 결과로는 첫번째 쿼리의 답과 두번째 쿼리의 답을 뺀거야. 최종 r부분에 답을 할때 첫번째 문장이름은 q_1, 첫번째 문장의 답 이름은 a_1이야. 그리고 찻반쩨 문장의 장소 이름은 site_1 로 해줘. 최종 답은 딱 다음과 같은 리스트의 형태로 똑같이 나와야해. [[q,오늘 우리집 온도 알려줘],[q,오늘 옆집 온도 알려줘],[o,final_result =a_1-a_2],[r, "final_result를 한국어로 답해줘"]] 야. 두번째 예시로는 지난 여름 우리집 평균온도 알려줘 라는 문장은 장소를 데이터베이스 테이블 이름이라고 할때  하나의 테이블(장소)에서 해결할 수 있는 세부문장이 없는 문장이야. 따라서 답은 다음 리스트이 형태 그대로 똑같이 나와야해. 변수 바꾸지마. [[q,지난 여름 우리집 평균온도 알려줘],[o,final_result=a_1],[r, "final_result를 한국어로 답해줘"]] 야.
예시를 하나 더 들면, 이번달 중 우리집 온도가 가장 추운달이 언제야 라는 문장은 하나의 테이블(장소)로 해결될수 있으니 답은 다음과 같이 똑같이 나와야해. 변수 바꾸지마. [q, 이번달 중 우리집 온도가 가장 추운달이 언제야][o,final_result=a_1],[r, "final_result를 한국어로 답해줘"]]. 예시를 하나 더 들면, 우리집과 오피스텔 101동중에 최고온도가 더 낮은 곳은 어디야? 의 세부문장 리스트 알려줘 라는 문장은
[
    [q, 우리집의 최고 온도가 얼마야?],
    [q, 오피스텔 101동의 최고 온도가 얼마야?],
    [o, final_result = "site_1" if a_1 < a_2 else "site_2"],
